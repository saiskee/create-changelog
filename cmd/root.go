...
func GetVersions(infos []fs.FileInfo) error {
  ...
  }, []string{createChangelogPrompt, "Check github for latest release", "Create my own changelog directory"})
  if answer == createChangelogPrompt {
    ...
    fmt.Printf("From github.com/%s/%s, the latest releases are %s", opts.Org, opts.Repo, strings.Join(r, ","))
    return nil
